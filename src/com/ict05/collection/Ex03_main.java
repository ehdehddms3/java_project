...
public class Ex03_main {
	...
	public static void main(String[] args) {
		...
		System.out.println("==========================");
		// 2. 컬렉션
		 HashSet<Ex03> h1 = new HashSet<>(); // coffee, cola, juice가 Ex03이라는 클래스로 이루어져있기 때문이다.(제네닉 타입)
		// 추가
		 h1.add(coffee);
		 h1.add(cola);
		 h1.add(juice);
		 // 더 추가할 수 있음 (배열은 할 수 없음)
		 h1.add(new Ex03("ion", 1500));
		 
		 // 꺼내기
		 // 1. 개선된 for문 사용                => 배열과 이름만 다르고 똑같이 사용한다.
		 for (Ex03 k : h1) {
				System.out.println("이름 : " + k.getName());
				System.out.println("가격 : "+ k.getPrice());
			}
		...
		// 갯수 : size
		 ...
		 // 삭제 : remove,clear()          => remove는 해당객체가 있으면 삭제, 없으면 삭제안함
		 // cola가 있으면 삭제 하시오
		 if (h1.contains(cola)) {
			 h1.remove(cola);
